"""Delete GCP Compute Node Groups."""
...
class Resource(BaseResource):
    ...
    def get_resources(self):
        ...
        for zone in self.zones:
            request = client.nodeGroups().list(project=self.project, zone=zone)# pylint: disable=maybe-no-member
    ...
    def delete_resources(self, resources, options=None):
        ...
        for resource in resources:
            try:
                response = client.nodeGroups().delete(# pylint: disable=maybe-no-member
                    project=self.project,
                    zone=resource['Zone'],
                    nodeGroup=resource['Id']
                ).execute()
                print(response)
