"""Delete GCP Compute Instance Groups."""
...
class Resource(BaseResource):
    """Base Resource Class."""
    name = 'gcp_compute_instancegroups'
    type = 'gcp'
    client = None
    credentials = None
    zones = []
    ...
    def get_resources(self):
        ...
        for zone in self.zones:
            request = client.instanceGroupManagers().list(project=self.project, zone=zone)# pylint: disable=maybe-no-member
    ...
    def delete_resources(self, resources, options=None):
        ...
        for resource in resources:
            try:
                response = client.instanceGroupManagers().delete(# pylint: disable=maybe-no-member
                    project=self.project,
                    zone=resource['Zone'],
                    instanceGroupManager=resource['Id']
                ).execute()
                print(response)
